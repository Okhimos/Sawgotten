	...
	if IsValid(player) and player:IsPlayer() then
		...
		if damageInfo:GetDamage() >= 15 then
			...
			if self.strikesRequired <= 0 then
				Clockwork.chatBox:AddInTargetRadius(player, "it", "Осадная лестница наконец не выдерживает и разваливается на несколько частей!", player:GetPos(), config.Get("talk_radius"):Get() * 2);
